          ...
          >
            ...
            {/* <div className="flex flex-wrap gap-4">
              <Link to="/packages/india">
                <Button size="lg" className="bg-white text-primary hover:bg-white/90 font-body font-semibold gap-3 px-8 py-4 rounded-full shadow-2xl hover:shadow-white/25 transition-all duration-300 hover:scale-105">
                  Explore India <ArrowRight className="w-5 h-5" />
                </Button>
              </Link>
              <Link to="/packages/nepal">
                <Button size="lg" className="bg-accent text-white hover:bg-accent/90 font-body font-semibold gap-3 px-8 py-4 rounded-full shadow-2xl hover:shadow-accent/25 transition-all duration-300 hover:scale-105">
                  Explore Nepal <ArrowRight className="w-5 h-5" />
                </Button>
              </Link>
              <Link to="/packages/south-korea">
                <Button size="lg" className="bg-blue-600 text-white hover:bg-blue-700 font-body font-semibold gap-3 px-8 py-4 rounded-full shadow-2xl hover:shadow-blue-600/25 transition-all duration-300 hover:scale-105">
                  Explore South Korea <ArrowRight className="w-5 h-5" />
                </Button>
              </Link>
            </div> */}
          </motion.div>
        ...
        <div className="container">
          ...
          >
            ...
            <p className="text-muted-foreground text-lg max-w-2xl mx-auto font-body">
              Explore the rich cultures and breathtaking landscapes of Asia's most captivating destinations, each offering unique experiences and memories.
            </p>
          </motion.div>
            ...
            {[
              ...
              <Link to={`/packages/${dest.slug}`} key={dest.slug} className="group">
                ...
                >
                  ...
                  {/* Country flag or icon overlay */}
                  <div className={`absolute top-4 right-4 w-12 h-12 bg-gradient-to-br ${dest.color} rounded-full flex items-center justify-center text-white font-bold text-lg shadow-lg`}>
                    {dest.name.charAt(0)}
                  </div>
                  
                  <div className="absolute bottom-0 left-0 right-0 p-8">
                    <h3 className="font-display text-3xl font-bold text-white mb-2 group-hover:text-accent transition-colors duration-300">
                      {dest.name}
                    </h3>
                    <p className="text-white/90 text-sm mb-4 font-body leading-relaxed">
                      {dest.desc}
                    </p>
        ...
        <div className="container">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
            {packageCategories.map((cat, index) => (
              ...
              >
                <Link to={`/packages/india?category=${cat.id}`} className="group">
      ...
      <section className="py-20 bg-gradient-to-b from-background via-muted/20 to-background">
        <div className="container">
          ...
          >
            ...
            <h2 className="font-display text-4xl md:text-5xl font-bold text-foreground mb-4">
              Popular <span className="text-accent">Tour Packages</span>
            </h2>
            <p className="text-muted-foreground text-lg font-body max-w-2xl mx-auto">
              Our most loved and highly-rated tour experiences, crafted for unforgettable adventures
            </p>
          </motion.div>
      ...
      <section className="py-20 bg-gradient-to-r from-primary via-primary to-accent relative overflow-hidden">
        ...
        <div className="container text-center relative z-10">
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <div className="inline-block px-6 py-3 bg-white/20 backdrop-blur-md rounded-full text-white text-sm font-medium mb-6 border border-white/20">
              🚀 Ready for Adventure?
            </div>
            <h2 className="font-display text-4xl md:text-5xl font-bold text-white mb-6">
              Can't Find Your Perfect Trip?
            </h2>
            <p className="text-white/90 text-lg mb-8 max-w-2xl mx-auto font-body leading-relaxed">
              We customize tours based on your unique preferences. Tell us your dream destination and we'll craft the perfect itinerary just for you!
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
              <EnquiryModal trigger={
                <Button size="lg" className="bg-white text-primary hover:bg-white/90 font-body font-semibold px-8 py-4 rounded-full shadow-2xl hover:shadow-white/25 transition-all duration-300 hover:scale-105">
                  Send an Enquiry
                </Button>
              } />
