...
const Carousel = ({ scrollRef, children }: { scrollRef: React.RefObject<HTMLDivElement>; children: React.ReactNode }) => (
  <div className="relative">
    ...
    <div
      ref={scrollRef}
      className="flex gap-5 overflow-x-auto py-20 px-14 scrollbar-none"
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <section className="relative min-h-[92vh] flex flex-col items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 container text-center">
          ...
          >
            ...
            {/* Headline */}
            <h1 className="font-display text-5xl sm:text-7xl md:text-8xl font-black text-white mb-6 leading-tight tracking-tight">
              {t("home.heroTitle")}
              <br />
            ...
            <p className="text-white/60 text-lg md:text-xl mb-10 max-w-2xl mx-auto font-body leading-relaxed">
              {t("home.heroSubtitle")}
            </p>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-white/10">
              {stats.map((stat, i) => (
                ...
                >
                  ...
                  <div className="text-2xl font-black text-primary font-display">{stat.value}</div>
      ...
      <section className="py-16 bg-background border-b border-border/40">
        <div className="container">
          ...
          />
          {featured.length === 0 ? (
            <p className="text-center text-muted-foreground py-12">{t("home.loadingPackages")}</p>
          ) : (
            <Carousel scrollRef={pkgRef}>
              {featured.map((pkg, index) => (
                <motion.div
                  ...
                  className="min-w-[340px] w-[340px] h-[400px] relative z-[1] hover:z-10 group/card"
                >
                  <div className="transition-transform duration-300 ease-out origin-center group-hover/card:scale-[1.25] h-[400px]">
                    <PackageCard pkg={pkg} />
                  </div>
                </motion.div>
              ))}
        ...
        <div className="container">
          ...
          <Carousel scrollRef={destRef}>
            {destinations.map((dest, index) => {
              ...
              return (
                <Link to={`/packages/${dest.slug}`} key={dest.slug} className="group min-w-[300px] w-[300px]">
                  <motion.div
                    initial={{ opacity: 0, x: 30 }}
                    whileInView={{ opacity: 1, x: 0 }}
                    viewport={{ once: true }}
                    transition={{ delay: index * 0.07 }}
                    className="relative h-[380px] rounded-2xl overflow-hidden border border-border/40 hover:border-primary/40 transition-all duration-500 hover:-translate-y-2"
                  >
            ...
            {categories.map((cat, index) => {
              ...
              return (
                <motion.div
                  ...
                  className="min-w-[260px] w-[260px] relative z-[1] hover:z-10 group/cat"
                >
                  <div className="transition-transform duration-300 ease-out origin-center group-hover/cat:scale-[1.25]">
                    <Link to={`/packages/india?category=${cat.slug}`} className="group block">
                      <div className={`relative h-[190px] rounded-2xl overflow-hidden bg-gradient-to-br ${ui.gradient} border border-white/5`}>
